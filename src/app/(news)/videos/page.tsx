import type { Metadata } from "next";
import { ArchiveLoadMore } from "@/components/organisms/ArchiveLoadMore";
import { getVideos } from "@/services/media.service";
import { buildPageMetadata } from "@/seo/metadata";
import { themeConfig } from "@/config/theme";

export const revalidate = 60;

export const metadata: Metadata = buildPageMetadata({
  title: "Videos",
  description: `Watch video reports from ${themeConfig.siteName}`,
  path: "/videos",
});

export default async function VideosIndexPage() {
  let nodes: Awaited<ReturnType<typeof getVideos>>["nodes"] = [];
  let pageInfo = { hasNextPage: false, endCursor: null as string | null };
  let error = false;

  try {
    const result = await getVideos({ first: 24 }, { revalidate: 60 });
    nodes = result.nodes ?? [];
    pageInfo = {
      hasNextPage: Boolean(result.pageInfo?.hasNextPage),
      endCursor: result.pageInfo?.endCursor ?? null,
    };
  } catch {
    error = true;
  }

  return (
    <div className="space-y-8">
      <header className="border-b border-[var(--np-border)] pb-4">
        <h1 className="font-heading text-3xl font-bold text-[var(--np-primary)]">
          Videos
        </h1>
        <p className="mt-2 text-[var(--np-muted)]">
          Video reports and explainers from the newsroom.
        </p>
      </header>

      {error ? (
        <p role="alert" className="text-sm text-[var(--np-accent)]">
          Videos are temporarily unavailable.
        </p>
      ) : (
        <ArchiveLoadMore
          kind="videos"
          initialItems={nodes}
          initialPageInfo={pageInfo}
          first={24}
          emptyMessage="No videos published yet."
        />
      )}
    </div>
  );
}
